    ...
    do
      {
        ...
        echo "Rscript ${SCRIPTDIR}/02.correlation_allmiRNAs_geneFPKM_transcriptionTested_withoutSVA.R $condition $perm"' $SLURM_ARRAY_TASK_ID'
      }>tempScript.sh
...
#         [[ $lastLine != '[1] "done"' ]] && sbatch -J TRTest_miRGene -p geh --qos geh --mem=80G --array=$i tempScript.sh 
        done
    rm tempScript.sh
    done
done
