...
get_cis_eqtls <- function(chromosome, condition){
  temp = fread(paste(EVO_IMMUNO_POP, "Maxime/miRNA_V2/data/09.mirQTL/cis_mirQTL/eqtls", chromosome, "_", condition, "_0_matrixeQTL_output.tsv", sep=""))
...
for (chr in 1:22){
  for (cond in 1:5){
    print(paste("cis eqtls :", chr, cond))
...
get_cis_permutations <- function(chromosome, condition, permutation_number){
  temp = fread(paste(EVO_IMMUNO_POP, "Maxime/miRNA_V2/data/09.mirQTL/cis_mirQTL/permutations/eqtls", chromosome, "_", condition, "_",permutation_number,"_matrixeQTL_output.tsv", sep=""))
...
pdf(paste(EVO_IMMUNO_POP, "Maxime/miRNA_V2/figures/09.mirQTL//FDR_pvalue_relationship_in_cis_mirQTL.pdf", sep=""))
print(p)
dev.off()
...
write.table(cis_eqtls, paste(EVO_IMMUNO_POP, "Maxime/miRNA_V2/data/09.mirQTL/cis_mirQTLs_with_FDR.tsv", sep=""), quote = F, row.names = F, sep="\t")

############################################################################
##We want to keep only the best association for each miRNA (condition wise)##
############################################################################
...
write.table(filtered_cis_eqtl, paste(EVO_IMMUNO_POP, "Maxime/miRNA_V2/data/09.mirQTL/cis_mirQTLs_with_FDR_filtered_best_miRNA_snp_association.tsv", sep=""), quote = F, row.names = F, sep="\t")
...
#   temp = fread(paste(EVO_IMMUNO_POP, "Martin/miRNA/07.eQTL/results/trans/eqtls", chromosome, "_", condition, "_0.tsv", sep=""))
#   return(temp)
# }
# trans_eqtls = list()
# for (chr in 1:22){
...
#   temp = fread(paste(EVO_IMMUNO_POP, "Martin/miRNA/07.eQTL/results/trans/permutations/eqtls", chromosome, "_", condition, "_",permutation_number,".tsv", sep=""))
#   temp[, type := paste("permutation", permutation_number)]
#   return(temp)
# }
# trans_perm = list()
...
# }
# to_plot = list()
# for (p in c(1e-2, 1e-3, 5e-3, 1e-4, 5e-4, 1e-5, 5e-5, 1e-6, 5e-6,1e-7,5e-7,1e-8,5e-8, 1e-9, 5e-9, 1e-10, 5e-10, 1e-11, 5e-11, 1e-12, 5e-12, 1e-13, 5e-13)){
#   to_plot[[as.character(p)]] = compute_fdr_for_pvalue_limit(p)
# }
# to_plot = rbindlist(to_plot)
# to_plot[, logpval := -log10(pvalue_limit)]
...
# for (p in sort(c(1e-2, 1e-3, 5e-3, 1e-4, 5e-4, 1e-5, 5e-5, 1e-6, 5e-6,1e-7,5e-7,1e-8,5e-8, 1e-9, 5e-9, 1e-10, 5e-10, 1e-11, 5e-11, 1e-12, 5e-12, 1e-13, 5e-13), decreasing = T)){
#   trans_eqtls[pvalue < p, FDR_Perm := to_plot[pvalue_limit == p, mean_fdr]]
# }
#
# write.table(trans_eqtls, "../results/trans_eqtls.tsv", quote = F, row.names = F, sep="\t")
