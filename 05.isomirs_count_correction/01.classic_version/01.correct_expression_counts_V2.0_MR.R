##########################################################################################
## This script aims to correct the raw miRNA transcripts counts with all substitutions  ##
##########################################################################################
...
#############################
raw_isomiR_counts=fread(sprintf("%s/Maxime/miRNA_V2/data/03b.isomirs_alignment/isomiR_counts.RPM.raw_23447_V2.0_MR.tsv",EVO_IMMUNO_POP))
...
isomir_counts_file_norm = isomir_counts_file_norm[mean_isoMiR > 1 & ratio_isoMiR>1]
...
write.table(iso_NT_combat_counts_file_gc_norm_3, sprintf("%s/Maxime/miRNA_V2/data/05.isomirs_count_correction/isomiR_counts.RPM.GCRL_Batch_lane_corrected.tsv",EVO_IMMUNO_POP), sep="\t", quote = F, row.names = T)
write.table(iso_NT_combat_counts_file_gc_norm_3_logged, sprintf("%s/Maxime/miRNA_V2/data/05.isomirs_count_correction/isomiR_counts.log2RPM.GCRL_Batch_lane_corrected.tsv",EVO_IMMUNO_POP), sep="\t", quote = F, row.names = T)
