...
###############################
plot_pca <-function(data_table, samples_to_get,PCx=1,PCy=2){
	suppressMessages(require("pcaMethods"))

#   pca<-prcomp(t(data_table[, mget(samples_to_get)]),center=T,scale=F)
	DATA=t(data_table[, mget(samples_to_get)])
	DATA[is.nan(DATA)]=NA
  PCs=pca(DATA,nPcs=10)
  pcs.prop.var<-round(PCs@R2*100,1)
  to_plot = as.data.table(PCs@scores[,c(PCx,PCy)])
  colnames(to_plot)=c('var1','var2')
  to_plot[, library_ID := samples_to_get]
  to_plot[, color := paste(substr(library_ID,8,8), substr(library_ID, 1,3), sep="_")]
    p <- ggplot(to_plot, aes(x=var1, y = var2, color = color))
  p <- p + geom_point()
  p <- p + scale_color_manual(values = colERC)
  p <- p + xlab(paste("PC",PCx," : ", pcs.prop.var[PCx], "%", sep=""))
  p <- p + ylab(paste("PC",PCy," : ", pcs.prop.var[PCy], "%", sep=""))
  p <- p + theme_bw()
  return(p)
}
...
write.table(iso_NT_combat_counts_file_gc_norm_3, sprintf("%s/Maxime/miRNA_V2/data/05.isomirs_count_correction/isomiR_counts_aggregated_nosubs.RPM.GCRL_Batch_lane_corrected.tsv",EVO_IMMUNO_POP), sep="\t", quote = F, row.names = T)
write.table(iso_NT_combat_counts_file_gc_norm_3_logged, sprintf("%s/Maxime/miRNA_V2/data/05.isomirs_count_correction/isomiR_counts_aggregated_nosubs.log2RPM.GCRL_Batch_lane_corrected.tsv",EVO_IMMUNO_POP), sep="\t", quote = F, row.names = T)
...
fwrite(isomir_ratio_cast,file=sprintf("%s/Maxime/miRNA_V2/data/05.isomirs_count_correction/isomiR_ratios_aggregated_nosubs.GCRL_Batch_lane_corrected.tsv",EVO_IMMUNO_POP), sep="\t")
dim(isomir_ratio_cast[-grep('other', isomir_ratio_cast$isomir_ID),]) 
# 2068  979
...
################################################
pdf(sprintf('%s/Maxime/miRNA_V2/figures/05.isomirs_count_correction/PCA_ajusted_isomiR_ratios_aggregated_nosubs_GCRL_Batch_lane_corrected.pdf',EVO_IMMUNO_POP),width=5,height=4)
test = as.data.table(isomir_ratio_cast[-grep('other', isomir_ratio_cast$isomir_ID),])
