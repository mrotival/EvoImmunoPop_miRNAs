    ...
    for record in SeqIO.parse(
        ...
        if record.id in miRNAs_names:
            if record.id in sequence_for_each_miRNA.keys():
                ...
                sequence_for_each_miRNA[record.id] = record.seq.back_transcribe()
...
def main():
    ...
    miRNA_fasta_file = "../temp_data/miRNA_DNA.fasta"
    ...
    transcript_fasta_file = "../temp_data/temp_transcript_DNA.fasta"
    get_RNA_of_each_transcript_in_fasta(transcript_fasta_file)

    # Launch miranda
    print("Running miRanda")
    output_miranda = "../results/miRNA_binding_sites_on_protein_coding_3primeUTR_DNATEST.txt"
    run(
        [
